...
namespace MatchThreeEngine
{
	...
	{
		public static void Swap(int x1, int y1, int x2, int y2, TileDataContainer[,] tiles)
		{
		...
		public static (TileDataContainer[], TileDataContainer[]) GetConnections(int originX, int originY, TileDataContainer[,] tiles)
		{
			...
			var horizontalConnections = new List<TileDataContainer>();
			var verticalConnections = new List<TileDataContainer>();
		...
		public static MatchingLogic FindBestMatch(TileDataContainer[,] tiles)
		{
			var bestMatch = default(MatchingLogic);
			...
			for (var y = 0; y < tiles.GetLength(1); y++)
			{
				for (var x = 0; x < tiles.GetLength(0); x++)
				{
					...
					var match = new MatchingLogic(tile, h, v);
		...
		public static List<MatchingLogic> FindAllMatches(TileDataContainer[,] tiles)
		{
			var matches = new List<MatchingLogic>();
		...
		public static MoveController FindMove(TileDataContainer[,] tiles)
		{
			var tilesCopy = (TileDataContainer[,])tiles.Clone();
			...
			for (var y = 0; y < height; y++)
			{
				for (var x = 0; x < width; x++)
				{
					for (byte d = 0; d <= 3; d++)
					{
						...
						if (FindBestMatch(tilesCopy) != null) return new MoveController(x, y, x2, y2);
		...
		public static MoveController FindBestMove(TileDataContainer[,] tiles)
		{
			var tilesCopy = (TileDataContainer[,])tiles.Clone();
			...
			var bestMove = default(MoveController);
			...
			for (var y = 0; y < height; y++)
			{
				for (var x = 0; x < width; x++)
				{
					for (byte d = 0; d <= 3; d++)
					{
						...
						{
							bestMove = new MoveController(x, y, x2, y2);
